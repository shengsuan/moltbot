...
async function promptManualModel(params: {
  ...
  const modelInput = await params.prompter.text({
    message: params.allowBlank ? "默认模型 (空白保持)" : "默认模型",
    initialValue: params.initialValue,
    placeholder: "provider/model",
    validate: params.allowBlank
      ? undefined
      : (value) => (normalizeOptionalString(value) ? undefined : "必需"),
  });
...
async function maybeFilterModelsByProvider(params: {
  ...
  if (shouldPromptProvider) {
    const selection = await params.prompter.select({
      message: "根据供应商过滤模型",
      options: [{ value: "*", label: "所有供应商" }, ...buildModelProviderFilterOptions(next)],
      searchable: true,
    });
    if (selection !== "*") {
...
export async function promptDefaultModel(
  ...
  if (
    ...
    if (includeManual) {
      options.push({ value: MANUAL_VALUE, label: "输入模型" });
    }
    options.push({
      value: BROWSE_VALUE,
      label: "浏览模型",
  ...
  const catalogProgress = params.prompter.progress("加载可用模型");
  ...
  if (includeManual) {
    options.push({ value: MANUAL_VALUE, label: "手动输入" });
  }
...
export async function promptModelAllowlist(params: {
  ...
  const allowlistProgress = params.prompter.progress("加载可用模型");
  let catalog: Awaited<ReturnType<typeof loadModelCatalog>>;
  try {
    catalog = await loadPickerModelCatalog(cfg);
  } finally {
