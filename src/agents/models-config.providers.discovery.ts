...
import { discoverShengSuanYunModels, SHENGSUANYUN_BASE_URL } from "./shengsuanyun-models.js";
...
export async function buildShengSuanYunProvider(): Promise<ProviderConfig> {
  const models = await discoverShengSuanYunModels();
  log.debug(`discovered ${models.length} ShengSuanYun models`);
  return {
