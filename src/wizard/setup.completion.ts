...
function formatReloadHint(shell: ShellCompletionStatus["shell"], profileHint: string): string {
  if (shell === "powershell") {
    return "重启您的 shell（或重新加载 PowerShell 配置文件）。";
  }
  return `重启您的 shell 或运行: source ${profileHint}`;
}
  ...
  if (!completionStatus.profileInstalled) {
    ...
    const shouldInstall =
      params.flow === "quickstart"
        ...
        : await params.prompter.confirm({
            message: `为 ${cliName} 启用 ${completionStatus.shell} shell 补全？`,
            initialValue: true,
          });
    ...
    if (!cacheGenerated) {
      await params.prompter.note(
        `生成补全缓存失败。稍后运行 \`${cliName} completion --install\`。`,
        "Shell 补全",
      );
      return;
    }
    ...
    await params.prompter.note(
      `Shell 补全已安装。${formatReloadHint(completionStatus.shell, profileHint)}`,
      "Shell 补全",
    );
  }
