...
function buildSearchEnvRef(config: OpenClawConfig, provider: SearchProvider): SecretRef {
  ...
  if (!envVar) {
    throw new Error(
      `搜索提供程序没有环境变量映射 "${provider}" at ${entry?.credentialPath ?? "未知路径"}  secret-input-mode=ref.`,
    );
  }
...
export async function setupSearch(
  ...
  await prompter.note(
    [
      "网络搜索使你的 agent 能在线查询必要信息",
      "选择服务提供商。有些服务提供商需要 API 密钥，有些则无需密钥。",
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        ...
        `Env var: ${ref.id}${envAvailable ? " (可用)" : ""}.`,
        ...(envAvailable ? [] : [`设置 ${ref.id} 到 Gateway 环境.`]),
        "Docs: https://docs.openclaw.ai/tools/web",
      ].join("\n"),
      "网络搜索",
    );
  ...
  const keyInput = await prompter.text({
    message: keyConfigured
      ? `${credentialLabel} (留空以保持当前)`
      : envAvailable
        ? `${credentialLabel} (留空以使用环境变量)`
        : credentialLabel,
    placeholder: keyConfigured ? "留空以保持当前" : entry.placeholder,
  });
  ...
  await prompter.note(
    [
      `未存储 ${credentialLabel} 将无法使用，除非你配置一个API key.`,
      `获取 API Key: ${entry.signupUrl}`,
      "Docs: https://docs.openclaw.ai/tools/web",
    ].join("\n"),
    "网络搜索",
  );
