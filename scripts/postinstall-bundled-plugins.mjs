...
export function pruneInstalledPackageDist(params = {}) {
  ...
  // Skip pruning if inventory file doesn't exist (expected during Docker builds before build step)
  const inventoryPath = join(packageRoot, DIST_INVENTORY_PATH);
  if (!pathExists(inventoryPath)) {
    log.log(`[postinstall] skipping dist pruning: ${DIST_INVENTORY_PATH} not found (expected during initial build)`);
    return [];
  }
