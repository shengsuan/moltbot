...
ensure_macos_default_node_active() {
    ...
    ui_error "Node.js v${NODE_DEFAULT_MAJOR} 已经安装 ，但是终端使用的是 ${active_version} (${active_path})"
    if [[ -n "$brew_node_prefix" ]]; then
        echo "将其添加到您的 shell 配置文件中并重新启动 shell："
        echo "  export PATH=\"${brew_node_prefix}/bin:\$PATH\""
    else
        echo "确保 Homebrew node@${NODE_DEFAULT_MAJOR} 是 PATH 中的第一个，然后重新运行安装程序。"
    fi
    return 1
}
...
install_node() {
    if [[ "$OS" == "macos" ]]; then
        ...
        if command -v pacman &> /dev/null || is_arch_linux; then
            ...
            ui_success "Node.js v${NODE_DEFAULT_MAJOR} 安装成功"
            print_active_node_paths || true
            return 0
        fi
        ...
        if command -v apt-get &> /dev/null; then
            ...
            ui_error "无法检测包管理器"
            echo "请手动安装 Node.js ${NODE_DEFAULT_MAJOR} (或 Node ${NODE_MIN_VERSION}+ 最低要求): https://nodejs.org"
            exit 1
        fi
