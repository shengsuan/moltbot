...
if (!openclawRoot) {
  ...
  for (const name of CLI_NAMES) {
    try {
      ...
      if (
        existsSync(join(candidate2, "package.json")) &&
        existsSync(join(candidate2, "plugin-sdk"))
      ) {
...
if (!openclawRoot) {
  ...
  console.error(
    "[postinstall-link-sdk] WARNING: could not find openclaw/clawdbot/moltbot global installation, symlink not created",
  );
  process.exit(0);
}
