...
function findOpenclawRoot(pluginRoot) {
  ...
  for (const name of CLI_NAMES) {
    try {
      ...
      if (
        fs.existsSync(path.join(c2, "package.json")) &&
        fs.existsSync(path.join(c2, "plugin-sdk"))
      )
        return c2;
    } catch {}
  }
...
function isLinkValid(linkTarget) {
  try {
    ...
    if (stat.isSymbolicLink()) {
      return (
        fs.existsSync(path.join(linkTarget, "dist", "plugin-sdk")) ||
...
function ensurePluginSdkSymlink(pluginRoot, tag) {
  ...
  try {
    ...
    if (!openclawRoot) {
      console.error(
        `${tag} WARNING: could not find openclaw global installation, symlink not created`,
      );
      return false;
    }
