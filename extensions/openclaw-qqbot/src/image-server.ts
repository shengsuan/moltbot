  ...
  try {
    ...
    if (contentLength) {
      ...
      if (declaredSize > maxSizeBytes) {
        console.error(
          `[image-server] Download rejected: Content-Length ${declaredSize} exceeds limit ${maxSizeBytes}`,
        );
        return null;
      }
    }
    ...
    while (true) {
      ...
      if (totalSize > maxSizeBytes) {
        reader.cancel();
        console.error(
          `[image-server] Download aborted: size ${totalSize} exceeds limit ${maxSizeBytes}`,
        );
        return null;
      }
      chunks.push(value);
    ...
    if (originalFilename) {
      ...
      const ext = contentType
        ? (getExtFromMime(contentType.split(";")[0]?.trim() ?? "") ?? "bin")
        : "bin";
    ...
    fs.writeFileSync(filePath, buffer);
    console.log(
      `[image-server] Downloaded file: ${filePath} (${buffer.length} bytes, ${Date.now()}ms)`,
    );
