...
export async function downloadFile(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    if (attempt > 0) {
      ...
      console.log(
        `[image-server] Retry ${attempt}/${maxRetries} after ${delayMs}ms: ${url.slice(0, 120)}`,
      );
      await new Promise((r) => setTimeout(r, delayMs));
    }

    const result = await downloadFileOnce(url, originalFilename, {
      timeoutMs,
      destDir,
      maxSizeBytes,
    });
    ...
    console.error(
      `[image-server] Attempt ${attempt + 1}/${maxRetries + 1} failed (retryable): ${result.error}`,
    );
  }
  ...
  try {
    ...
    if (maxSizeBytes > 0) {
      ...
      if (contentLength > 0 && contentLength > maxSizeBytes) {
        ...
        console.error(
          `[image-server] File too large (Content-Length: ${sizeMB}MB, limit: ${limitMB}MB): ${url}`,
        );
        return {
          filePath: null,
          error: `文件过大（${sizeMB}MB），超过了${limitMB}M的下载限制`,
          retryable: false,
        };
      }
    }
    ...
    if (err instanceof Error && err.name === "AbortError") {
      ...
      return {
        filePath: null,
        error: `下载超时（${Math.round(timeoutMs / 1000)}秒）`,
        retryable: true,
      };
    }
    // 大小超限错误 — 不可重试
