...
export function resolveAdminOpenId(
  ctx: Pick<AdminResolverContext, "accountId" | "appId" | "log">,
): string | undefined {
...
export function sendStartupGreetings(
  ...
  (async () => {
    ...
    if (!upgradeTargetOpenId) {
      ...
      ctx.log?.info(
        `[qqbot:${ctx.accountId}] Version changed but no upgrade-greeting-target, silently updating marker (trigger=${trigger})`,
      );
      return;
    }
    ...
    try {
      ctx.log?.info(
        `[qqbot:${ctx.accountId}] Sending startup greeting to upgrade-requester (trigger=${trigger}): "${plan.greeting}"`,
      );
      ...
      ctx.log?.info(
        `[qqbot:${ctx.accountId}] Sent startup greeting to upgrade-requester: ${upgradeTargetOpenId}`,
      );
    } catch (err) {
