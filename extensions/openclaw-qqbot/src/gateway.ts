...
async function handleInteractionCreate(params: {
  ...
  if (event.data?.type === INTERACTION_TYPE_CONFIG_QUERY) {
    ...
    const groupCfg = groupOpenid
      ? resolveGroupConfig(latestCfg as any, groupOpenid, account.accountId)
      : null;
    ...
    const mentionPatternsArr: string[] = resolveMentionPatterns(
      latestCfg as any,
      interactionAgentId,
    );
    ...
    log?.info(
      `[qqbot:${account.accountId}] Interaction ACK (type=${INTERACTION_TYPE_CONFIG_QUERY}) sent: ${event.id}, claw_cfg=${JSON.stringify(clawCfg)}`,
    );
    ...
    const clawCfgUpdate = (resolved as Record<string, unknown>)?.claw_cfg as
    ...
    if (changed) {
      ...
      log?.info(
        `[qqbot:${account.accountId}] Config updated via interaction ${event.id}: ${JSON.stringify({
    ...
    const updatedGroupCfg = groupOpenid
      ? resolveGroupConfig(latestCfg as any, groupOpenid, account.accountId)
      : null;
    const updatedRequireMention = updatedGroupCfg?.requireMention ?? true;
    const updatedRequireMentionMode: GroupActivationMode = updatedRequireMention
      ? "mention"
      : "always";
    const pluginVersion = getApiPluginVersion();
    const fwVersionRaw = getFrameworkVersion();
    ...
    log?.info(
      `[qqbot:${account.accountId}] Interaction ACK (type=${INTERACTION_TYPE_CONFIG_UPDATE}) sent: ${event.id}, claw_cfg=${JSON.stringify(ackClawCfg)}`,
    );
  } else {
    // 其他类型：普通 ACK
...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  try {
    ...
    if (pluginRuntime?.channel?.reply?.dispatchReplyWithBufferedBlockDispatcher) {
      ...
      log?.error(
        `[qqbot:${account.accountId}] ⚠️ Runtime preflight: dispatchReply API 不可用，AI 消息处理可能失败。请检查 openclaw 版本兼容性`,
      );
    }
  } catch (preflightErr) {
    log?.error(
      `[qqbot:${account.accountId}] ⚠️ Runtime preflight failed: ${preflightErr}. AI 消息处理可能失败`,
    );
  }
      ...
      const handleMessage = async (event: {
        ...
        if (event.type === "group" && event.groupOpenid) {
          ...
          if (!isGroupAllowed(cfg as any, event.groupOpenid, account.accountId)) {
            log?.info(
              `[qqbot:${account.accountId}] Group ${event.groupOpenid} not allowed by groupPolicy, skipping`,
            );
            return;
          }

          // 2. @检测（委托 mentions 适配器）
          const mentionPatternsForDetect: string[] = resolveMentionPatterns(
            cfg as any,
            route.agentId,
          );
          ...
          const gate = resolveGroupMessageGate({
            ignoreOtherMentions: resolveIgnoreOtherMentions(
              cfg as any,
              event.groupOpenid,
              account.accountId,
            ),
          ...
          if (gate.action === "drop_other_mention") {
            ...
            log?.info(
              `[qqbot:${account.accountId}] Group ${event.groupOpenid}: drop message (ignoreOtherMentions=true, other user mentioned, bot not mentioned)`,
            );
            return;
          }
          ...
          if (gate.action === "block_unauthorized_command") {
            // 未授权控制命令：静默拦截，不交给 AI
            log?.info(
              `[qqbot:${account.accountId}] Group ${event.groupOpenid}: blocked unauthorized control command from ${event.senderId}: ${contentForCommand.slice(0, 50)}`,
            );
            return;
          }
          ...
          if (gate.action === "skip_no_mention") {
            ...
            log?.info(
              `[qqbot:${account.accountId}] Group ${event.groupOpenid}: activation=${activation} (configRequireMention=${configRequireMention}) not mentioned, recorded to history (limit=${historyLimit}, cached=${(groupHistories.get(event.groupOpenid) ?? []).length}${historyAttachments ? `, attachments=${historyAttachments.length}` : ""})`,
            );
            return;
          }
        ...
        const senderPrefix =
          event.type === "group" && !isMergedMsg
            ? `[${event.senderName ? `${event.senderName} (${event.senderId})` : event.senderId}] `
            : "";
        const isAtYouTag = event.type === "group" ? (wasMentioned ? " (@你)" : "") : "";
        ...
        if (isMergedMsg && mergedMessages?.length) {
          ...
          const envelopeParts = preceding.map((m) => {
            ...
            const senderName = m.senderName
              ? m.senderName.includes(m.senderId)
                ? m.senderName
          ...
          const lastSenderName = lastMsg.senderName
            ? lastMsg.senderName.includes(lastMsg.senderId)
              ? lastMsg.senderName
              : `${lastMsg.senderName} (${lastMsg.senderId})`
            : lastMsg.senderId;
          ...
          // 命令直接透传，不注入上下文
          userMessage = senderPrefix
            ? `${senderPrefix}${quotePart}${userContent}${isAtYouTag}`
            : `${quotePart}${userContent}`;
        }
        let agentBody = userContent.startsWith("/") ? userContent : `${dynamicCtx}${userMessage}`;
        ...
        const mergedGroupSystemPrompt =
          [qqbotSystemInstruction, groupSystemPrompt].filter(Boolean).join("\n") || undefined;
        ...
        await runWithRequestContext({ target: qualifiedTarget }, async () => {
          try {
              ...
              if (toolMediaUrls.length > 0) {
                ...
                for (const mediaUrl of toolMediaUrls) {
                  try {
                    ...
                    log?.error(
                      `[qqbot:${account.accountId}] Tool fallback sendMedia failed: ${err}`,
                    );
                  }
                }
                return;
            ...
            log?.info(
              `[qqbot:${account.accountId}] Streaming ${useStreaming ? "enabled" : "disabled"} for ${targetType} message from ${event.senderId}`,
            );
            ...
            const createStreamingController = (): StreamingController => {
              const ctrl = new StreamingController({
                ...
                onReplyBoundary: async (newReplyText: string) => {
                  log?.info(
                    `[qqbot:${account.accountId}] Reply boundary: creating new StreamingController for new reply`,
                  );
            ...
            if (useStreaming) {
              log?.info(
                `[qqbot:${account.accountId}] Streaming mode enabled for ${targetType} target`,
              );
                  ...
                  deliver: async (
                    ...
                    if (info.kind === "tool") {
                      ...
                      if (hasBlockResponse && toolMediaUrls.length > 0) {
                        ...
                        const urlsToSend = toolMediaUrls.filter(
                          (url) => !blockDeliveredMediaUrls.has(url),
                        );
                        const skippedCount = toolMediaUrls.length - urlsToSend.length;
                        toolMediaUrls.length = 0;
                        if (urlsToSend.length === 0) {
                          log?.info(
                            `[qqbot:${account.accountId}] All ${skippedCount} tool media URL(s) already handled by block deliver, skipping`,
                          );
                          return;
                        }
                        log?.info(
                          `[qqbot:${account.accountId}] Block already sent, immediately forwarding ${urlsToSend.length} tool media URL(s) (deduped from block deliver)`,
                        );
                    ...
                    if (streamingController && !streamingController.isTerminalPhase) {
                      ...
                      const deliverPreview = (payload.text ?? "")
                        .slice(0, 40)
                        .replace(/\n/g, "\\n");
                      log?.debug?.(
                        `[qqbot:${account.accountId}] Streaming deliver entry, textLen=${deliverTextLen}, phase=${streamingController.currentPhase}, sentChunks=${streamingController.sentChunkCount_debug}, preview="${deliverPreview}"`,
                      );
                      try {
                        await streamingController.onDeliver(payload);
                        log?.debug?.(
                          `[qqbot:${account.accountId}] Streaming deliver done, phase=${streamingController.currentPhase}`,
                        );
                      } catch (err) {
                      ...
                      if (
                        event.type === "group" &&
                        (trimmedReply === "NO_REPLY" || trimmedReply === "[SKIP]")
                      ) {
                        log?.info(
                          `[qqbot:${account.accountId}] Model decided to skip group message (token=${trimmedReply}) from ${event.senderId}: ${event.content?.slice(0, 50)}`,
                        );
                        return;
                      }
                      ...
                      if (streamingController.shouldFallbackToStatic) {
                        log?.info(
                          `[qqbot:${account.accountId}] Streaming API unavailable, falling back to static for this deliver`,
                        );
                    ...
                    const executeDeliver = async (
                      deliverPayload: { text?: string; mediaUrls?: string[]; mediaUrl?: string },
                      _deliverInfo: { kind: string },
                    ) => {
                      // ============ 引用回复 ============
                      ...
                      if (deliverPayload.mediaUrls)
                        for (const u of deliverPayload.mediaUrls) blockDeliveredMediaUrls.add(u);
                  ...
                  onError: async (err: unknown) => {
                    ...
                    if (streamingController && !streamingController.isTerminalPhase) {
                      try {
                        ...
                        log?.error(
                          `[qqbot:${account.accountId}] Streaming onError failed: ${streamErr}`,
                        );
                      }
                      ...
                      if (streamingController.shouldFallbackToStatic) {
                        log?.info(
                          `[qqbot:${account.accountId}] Streaming onError: no chunk sent, falling back to static error handling`,
                        );
                        // 不 return，继续走普通错误处理
                      } else {
                        return;
                    ...
                    if (
                      errMsg.includes("Unable to resolve plugin runtime module") ||
                      errMsg.includes("root-alias.cjs")
                    ) {
                      log?.error(
                        `[qqbot:${account.accountId}] ⚠️ openclaw 框架 runtime 模块解析失败，可能是 openclaw 版本与 plugin-sdk 不兼容。请尝试: npm install -g openclaw@latest && openclaw gateway restart`,
                      );
                      await sendErrorMessage(
                        "⚠️ AI 服务暂时不可用：openclaw 框架运行时模块加载失败。\n\n请管理员执行：\nnpm install -g openclaw@latest\nopenclaw gateway restart\n\n斜杠命令（如 /bot-ping）不受影响。",
                      );
                      return;
                    }
                  ...
                  ...(streamingController
                    ? {
                        onPartialReply: async (payload: { text?: string }) => {
                          ...
                          log?.debug?.(
                            `[qqbot:${account.accountId}] onPartialReply called, textLen=${textLen}, phase=${streamingController!.currentPhase}, isTerminal=${streamingController!.isTerminalPhase}, preview="${preview}"`,
                          );
                          try {
                            await streamingController!.onPartialReply(payload);
                            log?.debug?.(
                              `[qqbot:${account.accountId}] onPartialReply done, phase=${streamingController!.currentPhase}`,
                            );
                          } catch (err) {
                            // StreamingController 内部已有重试，这里只打日志
                            log?.error(
                              `[qqbot:${account.accountId}] Streaming onPartialReply error: ${err}`,
                            );
                          }
                        },
                      }
            ...
            try {
              ...
              if (streamingController?.shouldFallbackToStatic) {
                log?.debug?.(
                  `[qqbot:${account.accountId}] Streaming was degraded to static mode (no chunk sent successfully)`,
                );
              }
            ...
            if (
              errStr.includes("Unable to resolve plugin runtime module") ||
              errStr.includes("root-alias.cjs")
            ) {
              try {
                await sendErrorMessage(
                  "⚠️ AI 服务暂时不可用：openclaw 框架运行时模块加载失败。\n\n请管理员执行：\nnpm install -g openclaw@latest\nopenclaw gateway restart\n\n斜杠命令（如 /bot-ping）不受影响。",
          ...
          switch (op) {
            ...
            case 0: // Dispatch
              ...
              if (t === "READY") {
                ...
                log?.info(
                  `[qqbot:${account.accountId}] Bot added to group: ${event.group_openid} by ${event.op_member_openid}`,
                );
                recordKnownUser({
                ...
                log?.info(
                  `[qqbot:${account.accountId}] Bot removed from group: ${event.group_openid} by ${event.op_member_openid}`,
                );
                ...
                log?.info(
                  `[qqbot:${account.accountId}] Group ${event.group_openid} rejected bot proactive messages (by ${event.op_member_openid})`,
                );
                ...
                log?.info(
                  `[qqbot:${account.accountId}] Group ${event.group_openid} accepted bot proactive messages (by ${event.op_member_openid})`,
                );
                ...
                const sceneDesc =
                  event.scene ??
                  (event.chat_type === 0 ? "guild" : event.chat_type === 1 ? "group" : "c2c");
                log?.info(
                  `[qqbot:${account.accountId}] Interaction: scene=${sceneDesc}, type=${event.data?.type}, button_id=${resolved?.button_id}, button_data=${resolved?.button_data}, user=${event.group_member_openid || event.user_openid || resolved?.user_id || "unknown"}`,
                );

                handleInteractionCreate({ event, account, cfg, log }).catch((err) => {
                  log?.error(
                    `[qqbot:${account.accountId}] Failed to handle interaction ${event.id}: ${err}`,
                  );
                });
              }
              break;
