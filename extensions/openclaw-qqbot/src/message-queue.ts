...
export function createMessageQueue(ctx: MessageQueueContext): MessageQueue {
  ...
  const drainGroupBatch = async (all: QueuedMessage[], peerId: string): Promise<void> => {
    ...
    for (const cmd of commands) {
      log?.info(
        `[qqbot:${accountId}] Processing command independently for ${peerId}: ${(cmd.content ?? "").trim().slice(0, 50)}`,
      );
    ...
    if (normal.length > 0) {
      ...
      if (normal.length > 1) {
        log?.info(
          `[qqbot:${accountId}] Merged ${normal.length} queued group messages for ${peerId} into one`,
        );
      }
  ...
  const drainUserQueue = async (peerId: string): Promise<void> => {
    ...
    if (activeUsers.size >= maxConcurrentUsers) {
      log?.info(
        `[qqbot:${accountId}] Max concurrent users (${maxConcurrentUsers}) reached, ${peerId} will wait`,
      );
      return;
    }
  ...
  const enqueue = (msg: QueuedMessage): void => {
    ...
    if (queue.length >= maxSize) {
      ...
      if (isGroup && dropped?.senderIsBot) {
        log?.info(
          `[qqbot:${accountId}] Queue full for ${peerId}, dropping bot message ${dropped.messageId}`,
        );
      } else {
        log?.error(
          `[qqbot:${accountId}] Queue full for ${peerId}, dropping oldest message ${dropped?.messageId}`,
        );
      }
    }
  ...
  const startProcessor = (handleMessageFn: (msg: QueuedMessage) => Promise<void>): void => {
    ...
    log?.info(
      `[qqbot:${accountId}] Message processor started (per-user concurrency, max ${maxConcurrentUsers} users)`,
    );
  };
