...
export async function chunkedUploadC2C(
  ...
  console.log(
    `${prefix} Starting chunked upload: file=${fileName}, size=${formatFileSize(fileSize)}, type=${fileType}`,
  );
  ...
  console.log(
    `${prefix} File hashes: md5=${hashes.md5}, sha1=${hashes.sha1}, md5_10m=${hashes.md5_10m}`,
  );

  // 3. 申请上传 → 获取 upload_id + block_size + 预签名链接
  const accessToken = await getAccessToken(appId, clientSecret);
  console.log(
    `${prefix} >>> Calling c2cUploadPrepare(fileType=${fileType}, fileName=${fileName}, fileSize=${fileSize}, md5=${hashes.md5}, sha1=${hashes.sha1}, md5_10m=${hashes.md5_10m})`,
  );
  ...
  console.log(
    `${prefix} Upload prepared: upload_id=${upload_id}, block_size=${formatFileSize(block_size)}, parts=${parts.length}, concurrency=${maxConcurrent}, retryTimeout=${retryTimeoutMs ? retryTimeoutMs / 1000 + "s" : "default"}`,
  );
  ...
  const uploadPart = async (part: { index: number; presigned_url: string }): Promise<void> => {
    ...
    console.log(
      `${prefix} Part ${partNum}/${parts.length}: uploading ${formatFileSize(length)} (offset=${offset}, md5=${md5Hex})`,
    );
    ...
    console.log(
      `${prefix} >>> Calling c2cUploadPartFinish(upload_id=${upload_id}, partIndex=${partIndex}, blockSize=${length}, md5=${md5Hex})`,
    );
    ...
    console.log(
      `${prefix} Part ${partNum}/${parts.length}: completed (${completedParts}/${parts.length})`,
    );
...
export async function chunkedUploadGroup(
  ...
  console.log(
    `${prefix} Starting chunked upload (group): file=${fileName}, size=${formatFileSize(fileSize)}, type=${fileType}`,
  );
  ...
  const accessToken = await getAccessToken(appId, clientSecret);
  console.log(
    `${prefix} >>> Calling groupUploadPrepare(fileType=${fileType}, fileName=${fileName}, fileSize=${fileSize}, md5=${hashes.md5}, sha1=${hashes.sha1}, md5_10m=${hashes.md5_10m})`,
  );
  let prepareResp: UploadPrepareResponse;
  try {
    prepareResp = await groupUploadPrepare(
      accessToken,
      groupId,
      fileType,
      fileName,
    ...
    if (err instanceof ApiError && err.bizCode === UPLOAD_PREPARE_FALLBACK_CODE) {
      console.warn(
        `${prefix} groupUploadPrepare hit fallback code ${UPLOAD_PREPARE_FALLBACK_CODE}`,
      );
      throw new UploadDailyLimitExceededError(filePath, fileSize, err.message);
    }
    throw err;
  ...
  const uploadPart = async (part: { index: number; presigned_url: string }): Promise<void> => {
    ...
    const token = await getAccessToken(appId, clientSecret);
    console.log(
      `${prefix} >>> Calling groupUploadPartFinish(upload_id=${upload_id}, partIndex=${partIndex}, blockSize=${length}, md5=${md5Hex})`,
    );
    await groupUploadPartFinish(
      token,
      groupId,
      upload_id,
      partIndex,
  ...
  for (let attempt = 0; attempt <= PART_UPLOAD_MAX_RETRIES; attempt++) {
    try {
      ...
      try {
        ...
        console.log(
          `${prefix} >>> PUT Part ${partIndex}/${totalParts}${attemptLabel}: url=${presignedUrl}, size=${data.length}`,
        );
        ...
        if (!response.ok) {
          ...
          console.error(
            `${prefix} <<< PUT Part ${partIndex}/${totalParts}: FAILED ${response.status} ${response.statusText} (${elapsed}ms, requestId=${requestId}) body=${body}`,
          );
          throw new Error(`COS PUT failed: ${response.status} ${response.statusText} - ${body}`);
        }

        console.log(
          `${prefix} <<< PUT Part ${partIndex}/${totalParts}: ${response.status} OK (${elapsed}ms, ETag=${etag}, requestId=${requestId})`,
        );
        return; // 成功
      } finally {
      ...
      if (attempt < PART_UPLOAD_MAX_RETRIES) {
        ...
        console.warn(
          `${prefix} Part ${partIndex}/${totalParts}: attempt ${attempt + 1} failed (${lastError.message}), retrying in ${delay}ms...`,
        );
 ...
 * 只遍历文件一次，内存友好
 */
async function computeFileHashes(filePath: string, fileSize: number): Promise<UploadPrepareHashes> {
