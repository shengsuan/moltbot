  ...
  await executeSendQueue(sendQueue, mediaSendCtx, {
    onSendText: async (textContent) => {
      await sendTextChunks(
        filterInternalMarkers(textContent),
        event,
        actx,
        sendWithRetry,
        consumeQuoteRef,
...
export async function sendPlainReply(
  ...
  if (toolMediaUrls.length > 0) {
    ...
    if (payloadUrls.size > 0) {
      ...
      if (filtered.length < before) {
        log?.info(
          `${prefix} Pre-dedup: removed ${before - filtered.length} payload media URL(s) from toolMediaUrls`,
        );
  ...
  if (toolMediaUrls.length > 0) {
    ...
    if (dedupedToolMedia.length < toolMediaUrls.length) {
      log?.info(
        `${prefix} Deduped tool media: ${toolMediaUrls.length} → ${dedupedToolMedia.length} (skipped ${toolMediaUrls.length - dedupedToolMedia.length} already sent via localMedia/collectedImages)`,
      );
    }
    if (dedupedToolMedia.length > 0) {
      log?.info(
        `${prefix} Forwarding ${dedupedToolMedia.length} tool-collected media URL(s) after block deliver`,
      );
