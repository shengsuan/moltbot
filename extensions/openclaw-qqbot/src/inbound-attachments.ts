...
export async function processAttachments(
  ...
  const downloadTasks = attachments.map(async (att) => {
    ...
    if (isVoice && wavUrl) {
      const wavResult = await downloadFile(wavUrl, undefined, {
        destDir: downloadDir,
        timeoutMs: INBOUND_DOWNLOAD_TIMEOUT_MS,
      });
      if (wavResult.filePath) {
        ...
        log?.error(
          `${prefix} Failed to download voice_wav_url (${wavResult.error}), falling back to original URL`,
        );
      }
    }

    if (!localPath) {
      const dlResult = await downloadFile(attUrl, att.filename, {
        destDir: downloadDir,
        timeoutMs: INBOUND_DOWNLOAD_TIMEOUT_MS,
      });
  ...
  const processTasks = downloadResults.map(
    async ({ att, attUrl, isVoice, localPath, audioPath, dlError }) => {
  ...
  for (const result of processResults) {
    ...
    if (result.type === "image" && result.localPath) {
      ...
      const hint = result.dlError?.includes("超时") ? "(图片下载超时)" : "(图片下载失败)";
      ...
      const hint = result.dlError?.includes("超时") ? "(下载超时)" : "(下载失败)";
