  ...
  for (let attempt = 0; attempt <= COMPLETE_UPLOAD_MAX_RETRIES; attempt++) {
    try {
      ...
      if (attempt < COMPLETE_UPLOAD_MAX_RETRIES) {
        ...
        console.warn(
          `[qqbot-api] CompleteUpload attempt ${attempt + 1} failed, retrying in ${delay}ms: ${lastError.message.slice(0, 200)}`,
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
  ...
  for (let attempt = 0; attempt <= PART_FINISH_MAX_RETRIES; attempt++) {
    try {
      ...
      if (isRetryableBizCode(err)) {
        ...
        console.warn(
          `[qqbot-api] PartFinish hit retryable bizCode=${(err as ApiError).bizCode}, entering persistent retry (timeout=${timeoutMs / 1000}s, interval=1s)...`,
        );
      ...
      if (attempt < PART_FINISH_MAX_RETRIES) {
        ...
        console.warn(
          `[qqbot-api] PartFinish attempt ${attempt + 1} failed, retrying in ${delay}ms: ${lastError.message.slice(0, 200)}`,
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
...
async function partFinishPersistentRetry(
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (!isRetryableBizCode(err)) {
        console.error(
          `[qqbot-api] PartFinish persistent retry: error is no longer retryable (bizCode=${(err as ApiError).bizCode ?? "N/A"}), aborting`,
        );
        throw lastError;
      }
      ...
      console.warn(
        `[qqbot-api] PartFinish persistent retry #${attempt}: bizCode=${(err as ApiError).bizCode}, retrying in ${actualDelay}ms (remaining=${Math.round(remaining / 1000)}s)`,
      );
  ...
  // 超时
  console.error(
    `[qqbot-api] PartFinish persistent retry timed out after ${timeoutMs / 1000}s (${attempt} attempts)`,
  );
  throw new Error(
    `upload_part_finish 持续重试超时（${timeoutMs / 1000}s, ${attempt} 次重试），中止上传`,
  );
}
...
export async function acknowledgeInteraction(
  ...
  await apiRequest(accessToken, "PUT", `/interactions/${interactionId}`, {
    code,
    ...(data ? { data } : {}),
  });
}
...
export async function sendGroupMessage(
  ...
  const msgSeq = msgId ? getNextMsgSeq(msgId) : 1;
  const body = buildMessageBody(content, msgId, msgSeq, messageReference);
  return sendAndNotify(accessToken, "POST", `/v2/groups/${groupOpenid}/messages`, body, {
    text: content,
  });
}
...
export async function c2cUploadPrepare(
  ...
  return apiRequest<UploadPrepareResponse>(
    accessToken,
    "POST",
    `/v2/users/${userId}/upload_prepare`,
    {
      file_type: fileType,
      file_name: fileName,
      file_size: fileSize,
...
export async function c2cUploadPartFinish(
  ...
  await partFinishWithRetry(
    accessToken,
    "POST",
    `/v2/users/${userId}/upload_part_finish`,
...
export async function c2cCompleteUpload(
  ...
  return completeUploadWithRetry(accessToken, "POST", `/v2/users/${userId}/files`, {
    upload_id: uploadId,
  });
}
...
export async function groupUploadPrepare(
  ...
  return apiRequest<UploadPrepareResponse>(
    accessToken,
    "POST",
    `/v2/groups/${groupId}/upload_prepare`,
    {
      file_type: fileType,
      file_name: fileName,
      file_size: fileSize,
...
export async function groupUploadPartFinish(
  ...
  await partFinishWithRetry(
    accessToken,
    "POST",
    `/v2/groups/${groupId}/upload_part_finish`,
    { upload_id: uploadId, part_index: partIndex, block_size: blockSize, md5 },
    retryTimeoutMs,
  );
}
...
export async function groupCompleteUpload(
  ...
  return completeUploadWithRetry(accessToken, "POST", `/v2/groups/${groupId}/files`, {
    upload_id: uploadId,
  });
}
