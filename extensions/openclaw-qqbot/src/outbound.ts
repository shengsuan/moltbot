...
export async function sendPhoto(
  ...
  if (isHttp) {
    ...
    const dl = await downloadToFallbackDir(
      mediaPath,
      prefix,
      "sendPhoto",
      ctx.account.appId,
  ...
  if (isData) {
    try {
      ...
      console.log(
        `${prefix} sendPhoto: Data URL decoded to ${localFile} (${buf.length} bytes), using chunked upload`,
      );
      const result = await chunkedUploadAndSend(
        ctx,
        localFile,
        MediaFileType.IMAGE,
        prefix,
...
async function sendVoiceFromLocal(
  ...
  try {
    ...
    if (!uploadPath) {
      ...
      console.warn(
        `${prefix} sendVoice: SILK conversion failed, uploading raw file via chunked upload`,
      );
    ...
    console.log(
      `${prefix} sendVoice: using chunked upload (${formatFileSize(uploadSize)})${needsTranscode ? " [transcoded]" : ""}`,
    );
...
async function chunkedUploadAndSend(
    ...
    try {
      const uploadResult = await chunkedUploadC2C(
        ...
        fileType,
        {
          ...
          onProgress: (progress) => {
            console.log(
              `${prefix} ${callerName}: chunked upload progress ${progress.completedParts}/${progress.totalParts} parts, ${formatFileSize(progress.uploadedBytes)}/${formatFileSize(progress.totalBytes)}`,
            );
          },
        },
      );
      ...
      const token = await getToken(ctx.account);
      const r = await sendC2CMediaMessage(
        token,
        ctx.targetId,
  ...
  if (ctx.targetType === "group") {
    ...
    try {
      ...
      const token = await getToken(ctx.account);
      const r = await sendGroupMediaMessage(
        token,
        ctx.targetId,
        uploadResult.file_info,
        ctx.replyToId,
...
async function downloadToFallbackDir(
  ...
  try {
    ...
    if (!result.filePath) {
      ...
      console.error(
        `${prefix} ${caller} fallback: download failed for ${httpUrl.slice(0, 80)} — ${errorMsg}`,
      );
...
async function sendFallbackLink(
  ...
  try {
    ...
    return {
      channel: "qqbot",
      error: `${caller}: 媒体发送失败 (${errorReason})，兜底链接也发送失败 (${fallbackMsg})`,
    };
  }
}
...
export async function sendMedia(ctx: MediaOutboundContext): Promise<OutboundResult> {
  ...
  if (isAudioFile(mediaUrl, mimeType)) {
    ...
    console.warn(
      `[qqbot] sendMedia: sendVoice failed (${voiceError}), falling back to sendDocument`,
    );
