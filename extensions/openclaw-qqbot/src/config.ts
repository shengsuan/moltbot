...
export function resolveMentionPatterns(cfg: OpenClawConfig, agentId?: string): string[] {
  ...
  if (agentId) {
    ...
    const entry = agents?.list?.find(
      (a) => a.id?.trim().toLowerCase() === agentId.trim().toLowerCase(),
    );
...
export function resolveGroupConfig(
  ...
  return {
    requireMention:
      specificCfg.requireMention ??
      wildcardCfg.requireMention ??
      DEFAULT_GROUP_CONFIG.requireMention,
    ignoreOtherMentions:
      specificCfg.ignoreOtherMentions ??
      wildcardCfg.ignoreOtherMentions ??
    ...
    historyLimit:
      specificCfg.historyLimit ?? wildcardCfg.historyLimit ?? DEFAULT_GROUP_CONFIG.historyLimit,
  };
}
