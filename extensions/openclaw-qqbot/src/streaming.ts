...
export class StreamingController {
  ...
  private async _doPartialReply(payload: { text?: string }): Promise<void> {
    ...
    if (this.lastRawFull && text.length > 0 && !text.startsWith(this.lastRawFull)) {
      this.logInfo(
        `onPartialReply: reply boundary detected — raw prefix mismatch (new len=${text.length}, prev len=${this.lastRawFull.length}), finalizing current controller`,
      );
      ...
      if (this.deps.onReplyBoundary) {
        this.logInfo(
          `onPartialReply: invoking onReplyBoundary callback with newText len=${text.length}`,
        );
        await this.deps.onReplyBoundary(text);
      } else {
        this.logWarn(
          `onPartialReply: reply boundary detected but no onReplyBoundary callback registered, new reply text will be lost`,
        );
      }
      return;
    }
    ...
    try {
      ...
      if (hasIncomplete) {
        ...
        if (safeText) {
          ...
          this.logDebug(
            `processMediaTags: sentIndex advanced to ${this.sentIndex} after ending stream for incomplete tag`,
          );
          this.resetStreamSession();
        }
