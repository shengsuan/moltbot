...
export class StreamingController {
    ...
    async doStartStreaming(textEndInFull) {
        try {
            ...
            const sessionText = this._pendingSessionText ?? this.lastNormalizedFull.slice(this.sentIndex, textEndInFull);
            const [safeText] = stripIncompleteMediaTag(sessionText);
            // 全空白文本 → 不开启流式，退回 idle
            if (!safeText?.trim()) {
...
function toMediaSendContext(ctx) {
    ...
    const qualifiedTarget = event.type === "group" ? `qqbot:group:${event.groupOpenid}` : `qqbot:c2c:${event.senderId}`;
    return {
        mediaTarget,
        qualifiedTarget,
        account,
