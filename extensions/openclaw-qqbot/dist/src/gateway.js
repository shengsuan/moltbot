...
async function handleInteractionCreate(params) {
    ...
    if (event.data?.type === INTERACTION_TYPE_CONFIG_QUERY) {
        ...
        const groupCfg = groupOpenid
            ? resolveGroupConfig(latestCfg, groupOpenid, account.accountId)
            : null;
        ...
        const updatedRequireMentionMode = updatedRequireMention
            ? "mention"
            : "always";
        const pluginVersion = getApiPluginVersion();
        const fwVersionRaw = getFrameworkVersion();
...
export async function startGateway(ctx) {
    ...
    const trySlashCommandOrEnqueue = async (msg) => {
        ...
        try {
            ...
            if (replyFile) {
                try {
                    const targetType = msg.type === "group"
                        ? "group"
                        : msg.type === "c2c" || msg.type === "dm"
                            ? "c2c"
                            : "channel";
                    const targetId = msg.type === "group"
    ...
    const connect = async () => {
        ...
        try {
            ...
            const handleMessage = async (event) => {
                ...
                if (voiceAttachmentPaths.length > 0 ||
                ...
                const commandAuthorized = allowAll ||
                    allowFromList.some((entry) => entry.toUpperCase() === event.senderId.toUpperCase());
                ...
                const fromAddress = event.type === "guild"
                ...
                await runWithRequestContext({ target: qualifiedTarget }, async () => {
                    try {
                        ...
                        // ============ 流式消息控制器 ============
                        const targetType = event.type === "c2c"
                            ? "c2c"
                            : event.type === "group"
                                ? "group"
                                : "channel";
                        ...
                        const dispatchPromise = pluginRuntime.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
                            ...
                            dispatcherOptions: {
                                ...
                                onError: async (err) => {
                                    ...
                                    if (errMsg.includes("Unable to resolve plugin runtime module") ||
                        ...
                        if (errStr.includes("Unable to resolve plugin runtime module") ||
