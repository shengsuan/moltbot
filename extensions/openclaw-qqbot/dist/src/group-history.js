        ...
        if (source) {
            ...
            switch (att.type) {
                case "image":
                    ...
                    break;
                case "voice":
                    parts.push(att.transcript ? `[语音消息（内容: "${att.transcript}"）]` : "[语音消息]");
                    break;
                case "video":
...
export function buildPendingHistoryContext(params) {
    ...
    return [HISTORY_CTX_START, historyText, "", HISTORY_CTX_END, params.currentMessage].join(lineBreak);
}
/**
...
export function buildMergedMessageContext(params) {
    ...
    const lineBreak = params.lineBreak ?? "\n";
    return [MERGED_CTX_START, precedingParts.join(lineBreak), MERGED_CTX_END, currentMessage].join(lineBreak);
}
/**
 * 清空指定群的历史缓存（回复完成后调用）。
