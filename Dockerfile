...
RUN --mount=type=cache,id=openclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    ...
    if [ -n "$OPENCLAW_INSTALL_BROWSER" ]; then \
      ...
      ACTUAL_CHROME=$(find /home/node/.cache/ms-playwright /root/.cache/ms-playwright -type f \( -name "chrome" -o -name "chromium-headless-shell" \) | head -n 1) && \
      if [ -z "$ACTUAL_CHROME" ]; then echo "Browser executable not found" >&2; exit 1; fi; \
