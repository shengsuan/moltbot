...
RUN --mount=type=cache,id=openclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      ca-certificates curl git hostname lsof openssl procps python3 tini openssh-server openssh-client nano gettext-base && \
...
COPY --from=runtime-assets /app/node-version.mjs .
...
RUN install -d -m 0755  /root/.config && \
    install -d -m 0700  \
      /root/.openclaw \
      /root/.openclaw/workspace \
      /root/.config/openclaw 
RUN openclaw plugins install @openclaw/feishu --accept-capabilities && \
  openclaw plugins install @wecom/wecom-openclaw-plugin@2026.7.2 --accept-capabilities && \
  openclaw plugins install @tencent-weixin/openclaw-weixin@2.4.6 --accept-capabilities && \
  openclaw plugins install @tencent-connect/openclaw-qqbot@2.0.1 --accept-capabilities
